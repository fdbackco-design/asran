...
export default function Reviews() {
  ...
  useEffect(() => {
    updateSEO({
      title: "고객 후기 - 아슬란 주방용품 | ASRAN",
      description: "아슬란 주방용품을 실제 사용하신 고객들의 진솔한 후기와 평점을 확인해보세요. 평균 4.87점의 높은 만족도.",
      keywords: "아슬란 후기, 고객 리뷰, 주방용품 평점, 사용 후기, 별점",
    });
  }, []);
  ...
  const averageRating = totalReviews > 0 
    ? allReviews.reduce((sum: number, review: Review) => sum + review.rating, 0) / totalReviews 
    : 0;

  const ratingDistribution = [5, 4, 3, 2, 1].map(rating => {
    const count = allReviews.filter((review: Review) => review.rating === rating).length;
    return {
      rating,
      count,
  ...
  const photoReviewsCount = allReviews.filter((review: Review) => 
    ...
    <div className="min-h-screen bg-asran-bg" data-testid="page-reviews">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl lg:text-4xl font-bold text-asran-gray mb-6">고객 후기</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            아슬란 제품을 실제 사용하신 고객들의 생생한 후기를 확인해보세요
          </p>
        </div>
          ...
          <CardContent className="p-8">
            <div className="grid md:grid-cols-3 gap-8 items-center">
              ...
              <div className="text-center">
                <div className="text-5xl font-bold text-asran-amber mb-2" data-testid="average-rating">
              ...
              {/* Rating Distribution */}
              <div>
                <h4 className="font-semibold text-asran-gray mb-4">별점 분포</h4>
                <div className="space-y-2">
                  {ratingDistribution.map(({ rating, count, percentage }) => (
                    <div key={rating} className="flex items-center">
                      <span className="text-sm text-gray-600 w-8">{rating}★</span>
                      <Progress value={percentage} className="flex-1 mx-3 h-2" />
              ...
              {/* Quick Stats */}
              <div>
                <h4 className="font-semibold text-asran-gray mb-4">리뷰 통계</h4>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <span className="text-gray-600">포토 리뷰</span>
                    <span className="font-semibold text-asran-gray">
                      {photoReviewsCount}개 ({Math.round((photoReviewsCount / totalReviews) * 100)}%)
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">평균 도움됨</span>
                    <span className="font-semibold text-asran-gray">
                      {Math.round(allReviews.reduce((sum: number, r: Review) => sum + r.helpful, 0) / totalReviews)}개
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">만족도</span>
                    <span className="font-semibold text-asran-amber">
                      {Math.round((allReviews.filter((r: Review) => r.rating >= 4).length / totalReviews) * 100)}%
                    </span>
                  </div>
                </div>
              </div>
        ...
        <Card className="mb-8">
          <CardContent className="p-6">
            <div className="grid md:grid-cols-5 gap-4">
              ...
              {/* Photo Filter */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">포토 리뷰</label>
                <Button
            ...
            {/* Active Filters */}
            {(selectedProduct !== "all" || selectedRating !== "all" || showPhotosOnly) && (
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center" data-testid="no-reviews">
              ...
              <h3 className="text-xl font-semibold text-gray-700 mb-2">조건에 맞는 리뷰가 없습니다</h3>
